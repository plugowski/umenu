...
menu.add_screen(MenuScreen('Main Menu')
                ...
                .add(SubMenuItem('Lights')
                     ...
                     .add(SubMenuItem('LEDs')
                          .add(ToggleItem('Turn ON', config.get_status, config.toggle, 3))))
                .add(SubMenuItem('Main Info')
                     .add(InfoItem('Status:', 'ok'))
                     .add(InfoItem('Temp:', '45.1')))
                .add(SubMenuItem('Settings')
                     .add(ToggleItem('Acceleration', config.get_status, config.toggle, 4))
                     .add(ToggleItem('Eco Mode', config.get_status, config.toggle, 5)))
                .add(SubMenuItem('Settings 2')
                     .add(InfoItem('Config:', 'load'))
                     .add(InfoItem('Something:', 'show'))
                     .add(ToggleItem('Eco Mode', config.get_status, config.toggle, 5)))
                )
